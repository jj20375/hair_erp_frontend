        ...
        <el-table :data="datas"
            ...
            <el-table-column width="350"
                ...
                <template v-slot="scope">
                    ...
                    <div class="font-weight-bold text-dark">設計師:<span class="ml-2">{{ scope.row.user_designer.account }} ({{ scope.row.user_designer.name }})</span></div>
                    <div class="font-weight-bold text-primary">會員:<span class="ml-2">{{ scope.row.user_member.account }} ({{ scope.row.user_member.name }})</span></div>
                    <div class="font-weight-bold red-color">日期:<span class="ml-2">{{ scope.row.order_date | formatDate }}</span></div>
                </template>
            </el-table-column>
            <el-table-column min-width="200"
                ...
                <template v-slot="scope">
                    <ul>
                        <li v-for="(item, index) in scope.row.operator_info_order"
                            :key="index"
                            class="borderBottomWidth1 grayBorder">
                            <div class="font-weight-bold hot-color">標題:{{ item.title }}</div>
                            <div class="font-weight-bold dark-color">內容:{{ item.order_able.name }}</div>
                        </li>
                    </ul>
                </template>
            </el-table-column>
            <el-table-column width="200"
                             label="金額">
                <template v-slot="scope">
                    <div class="font-weight-bold hot-color">自訂義結帳金額:<span class="ml-2">{{ scope.row.preferential_price | formatCurrency }}</span></div>
                    <div class="font-weight-bold gray-color">折扣價:<span class="ml-2">{{ scope.row.info_total | formatCurrency }}</span></div>
                    <div class="font-weight-bold red-color">原價:<span class="ml-2">{{ scope.row.total | formatCurrency }}</span></div>
                </template>
            </el-table-column>
            <el-table-column width="200"
                             label="商家訂單創建者"
            ...
            <el-table-column min-width="150"
                             label="操作人員">
                <template v-slot="scope">
                    <div class="font-weight-bold hot-color">名稱:<span class="ml-2">{{ scope.row.user.userable.name }}</span></div>
                    <div class="font-weight-bold red-color">帳號:<span class="ml-2">{{ scope.row.user.userable.account }}</span></div>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="創建與更新時間">
...
/**
 * OperatorOrderListApi = 取得商家訂單列表api
 * OperatorOrderListByIdApi = 取得指定id商家訂單列表api
 */
import { OperatorOrderListApi, OperatorOrderListByIdApi } from "../../../api/operatorOrderApi";
...
export default {
    ...
    methods: {
        ...
        resetData() {
            ...
            if (self.$route.params.operatorId !== undefined) {
                self.getListsById({
                    ...
                    sort: ["operator_order_id|DESC"]
                });
            } else {
                self.getLists({ sort: ["operator_order_id|DESC"] });
            }
        },
        // 搜尋條件
        filterSearch() {
            let self = this;
            // 判斷是否有值 有值就帶入搜尋條件
            checkHaveSelectData(["operator_order_id|DESC", self.searchOrder, self.startTime, self.endTime], ["sort", "order_number", "start|order_date", "end|order_date"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
        ...
        async getLists(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await OperatorOrderListApi(sendData);
        ...
        async getListsById(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await OperatorOrderListByIdApi(sendData);
                self.datas = data.data.data;
                self.currentPage = data.data.current_page;
                self.paginationData.total = data.data.total;
                // 當pageSize 為 true 時 將預設一頁呈現幾筆值 改為傳入參數值
    ...
    created() {
        ...
        if (self.user.group.group_code !== "SYSYEM" && self.$route.params.operatorId === undefined) {
