<template>
    <div>
        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-3">
                <label for="">產品代碼</label>
                <el-input placeholder="請輸入產品代碼"
                          v-model="searchKey"
                          clearable
                          prefix-icon="el-icon-search">
                </el-input>
        ...
        <el-table :data="datas"
            ...
            <el-table-column min-width="150"
                ...
                <template v-slot="scope">
                    ...
                    <div class="borderTopWidth1 grayBorder"></div>
                    <div class="font-weight-bold hot-color">廠商:<span class="ml-2">{{ scope.row.provider.name }}</span></div>
                    <div class="font-weight-bold gray-color">廠商代碼:<span class="ml-2">{{ scope.row.provider.key }}</span></div>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="價格/庫存">
                <template v-slot="scope">
                    <div class="font-weight-bold hot-color">庫存:<span class="ml-2">{{ scope.row.quantity | formatCurrency }}</span></div>
                    <div class="font-weight-bold red-color">價格:<span class="ml-2">{{ scope.row.price | formatCurrency }}</span></div>
                    <badge type="primary">
                        <el-switch style="display: block;"
                                   v-model="scope.row.is_commodity"
                                   ...
                                   :inactive-value="0"
                                   inactive-text="非商品"
                                   @change="onChangeToCommodity(scope.row, scope.$index)">
                        </el-switch>
                    </badge>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="分類/標籤">
                <template v-slot="scope">
                    <div class="font-weight-bold hot-color">分類:<span class="ml-2">{{ scope.row.category.name }}</span></div>
                    <div class="font-weight-bold gray-color">分類代碼:<span class="ml-2">{{ scope.row.category.key }}</span></div>
                    <div class="borderTopWidth1 grayBorder mb-2"></div>
                    <badge type="warning"
                           class="ml-1"
                           v-for="(item,index) in scope.row.morph_to_many_tag"
                           :key="index">{{ item.name }}</badge>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
                             label="產品創建者"
...
/**
 * ProductListApi = 取得產品列表api
 * ProductListByIdApi = 取得指定id產品列表api
 * UpdateProductApi = 更新產品 api
 */
import { ProductListApi, ProductListByIdApi, UpdateProductApi } from "../../api/productApi";
...
export default {
    ...
    methods: {
         ...
         * @example { prodcut_id: products表id, is_commodity: 判斷是否為商品 }
         * @param { type Number(數字) } index 索引值
         */
        async onChangeToCommodity(data, index) {
            let self = this;
            // 表單發送資料
            let sendData = {
            ...
            try {
                ...
                if (value) {
                    try {
                        await UpdateProductApi(sendData);
                        self.$message({
                            type: "success",
                            message: "更新成功"
                        });
                        self.closeDialog();
                        self.datas[index].is_commodity = data.is_commodity;
                    } catch (err) {
                        self.$message({
                            type: "error",
                            message: "更新失敗"
                        });
                        self.datas[index].is_commodity = data.is_commodity === 1 ? 0 : 1;
                    }
                } else {
                    self.datas[index].is_commodity = data.is_commodity === 1 ? 0 : 1;
                }
         ...
         * 開關產品啟用狀態
         * @param { type Object(物件) } data 列表資料
         * @example { prodcut_id: products表id, active: 啟用狀態 }
         * @param { type Number(數字) } index 索引值
         */
        ...
        resetData() {
            ...
            if (self.$route.params.operatorId !== undefined) {
                ...
                self.getLists({ sort: ["product_id|DESC"] });
            }
        },
        // 搜尋條件
        filterSearch() {
            let self = this;
            // 判斷是否有值 有值就帶入搜尋條件
            checkHaveSelectData(["product_id|DESC", self.searchName, self.searchKey, self.searchStatus, self.startTime, self.endTime], ["sort", "name", "key", "status", "startDate", "endDate"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
        ...
        async getLists(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await ProductListApi(sendData);
        ...
        async getListsById(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await ProductListByIdApi(sendData);
                self.datas = data.data.data;
                self.currentPage = data.data.current_page;
                self.paginationData.total = data.data.total;
                // 當pageSize 為 true 時 將預設一頁呈現幾筆值 改為傳入參數值
