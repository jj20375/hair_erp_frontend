    ...
    <div>
        ...
        <div class="row justify-content-center">
            <div class="col-md-4 col-lg-6"
                 v-for="(operator,key) in datas"
                 :key="key"
                 v-loading="loading">
                <card>
                    <h2 class="card-title">{{ operator.name }}</h2>
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                            <thead class="hotBg text-center">
                                <tr class="text-center py-1">
                                    <th class="text-white">#</th>
                                    <th class="text-white">分類</th>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in datas[key]['data']"
                                    ...
                                    <td>{{ index +1 }}</td>
                                    <td>{{ item.label }}</td>
                                    <td class="text-right font-weight-bold red-color">{{ item.value | formatCurrency }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
/**
 * ReportTotalApi = 總額報表api
 */
import { ReportTotalApi } from "../../../api/reportApi";
...
export default {
    ...
    methods: {
        ...
        filterSearch() {
            ...
            // 判斷是否有值 有值就帶入搜尋條件
            checkHaveSelectData(["operator_order_id|DESC", self.searchOperator, self.startTime, self.endTime], ["sort", "operator_id", "customStartDate|order_date", "customEndDate|order_date"], self.filterData);
        ...
        async getReport(searchData) {
            ...
            self.loading = true;
            try {
                let { data } = await ReportTotalApi(searchData);
