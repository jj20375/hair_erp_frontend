        ...
        <el-table :data="datas"
            ...
            <el-table-column min-width="150"
                ...
                <template v-slot="scope">
                    ...
                    <div class="font-weight-bold text-primary">數量:<span class="ml-2">{{ scope.row.morph_to_many_product | checkQuantity }}</span></div>
            ...
            <el-table-column min-width="150"
                ...
                <template v-slot="scope">
                    <badge type="warning"
                           class="ml-1"
                           v-for="(item,index) in scope.row.morph_to_many_product"
                           :key="index">{{ item.name }}</badge>
                </template>
            </el-table-column>
            <el-table-column min-width="150"
...
/**
 * CommodityListApi = 取得商品列表api
 * CommodityListByIdApi = 取得指定id商品列表api
 * UpdateCommodityApi = 更新商品 api
 */
import { CommodityListApi, CommodityListByIdApi, UpdateCommodityApi } from "../../api/commodityApi";
...
export default {
    ...
    filters: {
        checkQuantity(val) {
            let quantity = _.map(val, "quantity");
            return _.min(quantity);
        }
    },
    computed: {
    ...
    methods: {
        ...
        async onActive(data, index) {
            ...
            try {
                ...
                if (value) {
                    try {
                        await UpdateCommodityApi(sendData);
                        self.$message({
                            type: "success",
                            message: "更新成功"
                        });
        ...
        resetData() {
            ...
            if (self.$route.params.operatorId !== undefined) {
                self.getListsById({
                    ...
                    sort: ["commodity_id|DESC"]
                });
            } else {
                self.getLists({ sort: ["commodity_id|DESC"] });
            }
        },
        // 搜尋條件
        filterSearch() {
            let self = this;
            // 判斷是否有值 有值就帶入搜尋條件
            checkHaveSelectData(["commodity_id|DESC", self.searchName, self.searchKey, self.searchStatus, self.startTime, self.endTime], ["sort", "name", "key", "status", "startDate", "endDate"], self.filterData);
            // 搜尋完後將分頁改回第一頁
            self.currentPage = 1;
            // 搜尋完後將分頁改回第一頁
        ...
        async getLists(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await CommodityListApi(sendData);
        ...
        async getListsById(sendData, pageSize = false) {
            ...
            self.loading = true;
            try {
                let { data } = await CommodityListByIdApi(sendData);
                self.datas = data.data.data;
                self.currentPage = data.data.current_page;
                self.paginationData.total = data.data.total;
                // 當pageSize 為 true 時 將預設一頁呈現幾筆值 改為傳入參數值
    ...
    created() {
        ...
        // 判斷非系統使用者 且網址上沒有傳送 operatorId 參數
